...
export class GameplayScene {
  ...
  handleMouseUp = (event) => {
    ...
    if (
      ...
      const isCorrect = this.stoneHandler.isDroppedStoneCorrect(
        self.pickedStone.text
      );
      console.log("LevelData->", this.levelData);
      let loadPuzzleData = { isCorrect: isCorrect };
      const dropStoneEvent = new CustomEvent("stonesdropped", {
  ...
  addEventListeners() {
    ...
    this.handler.addEventListener("touchmove", this.handleMouseMove, false);
  ...
  loadPuzzle = () => {
    ...
    if (this.counter == 1) {
      this.switchSceneToEnd(this.levelData, 3);
    } else {
